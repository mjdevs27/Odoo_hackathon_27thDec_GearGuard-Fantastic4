...
const MaintenanceRequestPage = () => {
    ...
    const handleStageChange = async (newStage: string) => {
        setFormData(prev => ({ ...prev, stage: newStage as MaintenanceRequest['stage'] }));
        if (!isNew && id) {
            try {
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div className="flex-1">
                                    <select
                                        ...
                                        onChange={(e) => {
                                            const value = e.target.value as 'equipment' | 'work_center';
                                            if (value === 'work_center') {
                                                // Route to Work Centers page when Work Center is selected
                                                navigate('/dashboard/work-centers');
                                            } else {
                                                setFormData(prev => ({
                                                    ...prev,
                                                    maintenance_for: value,
                                                    equipment_id: null,
                                                    work_center_id: null
                                                }));
                                            }
                                ...
                                <div className="flex-1 flex items-center gap-6">
                                    <label className="flex items-center gap-2 cursor-pointer">
                                        <input
                                            ...
                                            onChange={(e) => setFormData(prev => ({ ...prev, maintenance_type: e.target.value as 'CORRECTIVE' | 'PREVENTIVE' }))}
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div className="flex-1 flex items-center gap-2">
                                    {['LOW', 'MEDIUM', 'HIGH', 'URGENT'].map((priority, index) => (
                                        <button
                                            key={priority}
                                            onClick={() => setFormData(prev => ({ ...prev, priority: priority as MaintenanceRequest['priority'] }))}
